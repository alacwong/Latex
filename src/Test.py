...
# playground to mess with google cloud api
from pprint import pprint
import pandas as pd
import src.image.image_process as im

os.environ['GOOGLE_APPLICATION_CREDENTIALS'] = r'image//credentials.json'
...
def draw_boxes(image, bounds):
    ...
    for i in range(len(bounds)):
        draw.polygon([
            ...
            bounds[i].normalized_vertices[3].x * width, bounds[i].normalized_vertices[3].y * height], None, l[i % len(l)])
    return image


def detect_multiple(content):
    bounds = []
    image = vision.types.Image(content=content)
    ...
    for obj in objects:
        ...
        bounds.append(obj.bounding_poly)

    return bounds


client = vision.ImageAnnotatorClient()
image = get_image_pil(0, 2)
box = im.draw_boxes(image ,detect_multiple(im.image_to_byte(image)))
box.show()
